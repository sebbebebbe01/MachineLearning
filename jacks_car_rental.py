...
# T = np.zeros((N_cars+1,N_cars+1,N_cars+1,N_cars+1))
...
#         site1_after_loss = np.flip(demand_1[:n1+1]).copy() # Excluding the trailing zeros up to element nr N_cars+1
...
#         site2_after_loss = np.flip(demand_2[:n2+1]).copy()
    ...
    for n1 in range(N_cars+1):
        for n2 in range(N_cars+1):
            ...
            for a in range(min_a, max_a + 1):
                ...
                # Adaptation 1: free_shuttles free moves from site 1 to site 2
                if a >= 0:
                    cost = move*abs(max(a - free_shuttles, 0))
                else:
                    cost = move*abs(a)
                # Adaptation 2: parking fees if you exceed excess_park on either site
                if n1_prime > excess_park:
